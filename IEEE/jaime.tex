...
\author{Jaime Arturo Hurtado Romero\inst{1} \and Tomas Acosta Bernal \and 
...
spoofing attacks enabled by advances in deep generative modelling.  In
particular, a critical yet under-studied limitation is the severe performance
degradation that occurs when anti-spoofing countermeasure models trained on
English corpora are applied to Spanish speech, owing to systematic
accent-induced differences in prosodic and spectral features.  This paper
presents a systematic evaluation of 48 anti-spoofing architectures formed by
crossing four front-end feature extractors---Mel-frequency Cepstral
Coefficients (MFCC), Linear Frequency Cepstral Coefficients (LFCC),
Constant-Q Cepstral Coefficients (CQCC), and Spectrogram---with three Light
Convolutional Neural Network (LCNN) back-ends (Attention, Trim-pad, LSTM-sum)
and four loss functions.  Models are trained and evaluated on the
ASVspoof~2019 English database, the HABLA Spanish multi-accent database, and
a combined bilingual corpus; each configuration is run six times with
different random seeds, and results are reported as mean Equal Error Rate
(EER).  Joint English--Spanish training achieves near-zero EER on
Spanish---the best result being MFCC-LCNN-LSTM-sum with AM-Softmax at
$0.09\%$---while sacrificing fewer than one percentage point on English,
demonstrating that a single bilingually trained model can generalise across
languages.  An accent-granularity analysis over five Latin-American accents
further reveals that the optimal architecture varies per accent, motivating
accent-adaptive or ensemble strategies in future work.
\end{abstract}
...
The \textit{ASVspoof} challenge series \cite{yamagishi2019asvspoof} \cite{delgado2021asvspoof} has driven most recent progress in anti-spoofing;
...
\begin{enumerate}
  ...
  \item A filter-bank scale analysis demonstrating that descendant
        ...
        front-ends, in line with the importance of low-frequency voice
        formants.
\end{enumerate}
...
AM-Softmax} at $\mathbf{0.09\%}$ ($\sigma=0.04$).  CQCC also performs well ($\leq 1.3\%$); Spectrogram remains the weakest
front-end, mirroring the Spanish-only experiment.
...
median EER, in line with the observation that human voice formants F1 and~F2
...
best median EER of $2.58\%$ on the English test set, while the descendant bank achieves a competitive median of $2.51\%$ with
marginally higher inter-run variance.  Table~\ref{table:scale-best-eer} summarises the variance
...
dataset~\cite{delgado2021asvspoof}, and the effect of speaker gender and age
