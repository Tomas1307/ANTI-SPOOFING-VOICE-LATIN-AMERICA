...
training samples. Chatterbox voice cloning requires only the reference audio
file path — no text transcription of the reference is needed.

The target duration is 10 seconds because Chatterbox internally clips reference
audio to approximately 10 seconds for decoder conditioning.
"""
import json
...
from app.pipeline.chatterbox_attack.utils.audio_concatenation import concatenate_with_padding


class ReferenceAudioPreparator:
    """Prepares reference audio clips for Chatterbox voice cloning.

    Concatenates multiple training samples per speaker to create reference
    clips used as audio_prompt_path in ChatterboxMultilingualTTS.generate().
    No transcription is required; Chatterbox extracts speaker conditioning
    ...
    Attributes:
        ...
        target_duration: Target duration in seconds for concatenated clips.
    """
    ...
    def execute(self) -> ReferenceResult:
        ...
        """
        logger.info("Step 1: Preparing reference audio clips for Chatterbox...")
