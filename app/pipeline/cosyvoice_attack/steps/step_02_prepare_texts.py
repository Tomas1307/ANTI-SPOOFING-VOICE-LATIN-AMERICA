...
Text prompts are filtered by word count range and assigned with reproducible seeding.
"""
import json
import numpy as np
import pandas as pd
...
class TextPromptPreparator:
    """Prepares Spanish text prompts for CosyVoice TTS generation.

    Loads Mozilla Common Voice transcripts and assigns random texts
    to each speaker with reproducible seeding. Texts are filtered
    to the configured word count range (default 5-50 words).
        ...
        for speaker_id in speaker_ids:
            ...
            for text in speaker_texts:
                speaker_prompts.append({
                    "text_id": f"COSYVOICE_TEXT_{text_counter:05d}",
                    "text": text,
