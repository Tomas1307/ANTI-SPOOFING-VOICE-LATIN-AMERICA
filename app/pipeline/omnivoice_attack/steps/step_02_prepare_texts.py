...
Assigns Spanish text prompts to each speaker from Mozilla Common Voice
transcripts. Texts are filtered by length (5-100 words) and sampled with
...
from app.pipeline.omnivoice_attack.schemas.text_prompts_result import TextPromptsResult


class TextPromptPreparator:
    """Prepares Spanish text prompts for OmniVoice TTS generation.

    Loads Mozilla Common Voice transcripts, filters by word-length range,
    and assigns texts to each speaker. Sampling is reproducible via the
    configured random seed. When MATCH_BONAFIDE_COUNT is enabled, each
    speaker receives as many prompts as their bonafide_count to keep the
    spoofed pool balanced against bonafide.

    Attributes:
        cv_metadata_path: Path to Mozilla Common Voice validated.tsv.
        output_dir: Directory containing reference metadata and where prompts are saved.
        samples_per_speaker: Texts per speaker when MATCH_BONAFIDE_COUNT is False.
    ...
    def execute(self) -> TextPromptsResult:
        ...
        Raises:
            FileNotFoundError: If reference_metadata.json or CV metadata is missing.
        """
        ...
        for speaker_id in speaker_ids:
            ...
            for text in speaker_texts:
                speaker_prompts.append({
                    "text_id": f"OMNIVOICE_TEXT_{text_counter:05d}",
                    "text": text,
