...
Uses OMNIVOICE system ID and 12000000+ audio ID range to avoid collisions
with FishGram (9000000+), Qwen (8000000+), OpenVoice (7000000+),
Chatterbox (6000000+), OuteTTS (10000000+), and CosyVoice (11000000+).

Audio is loaded with librosa at settings.SAMPLE_RATE (16 kHz), which
transparently resamples OmniVoice's native 24 kHz output before writing
to FLAC.
"""
import json
...
from app.pipeline.omnivoice_attack.schemas.formatting_result import FormattingResult
...
class OutputFormatter:
    ...
    following ASVspoof2019 LA naming conventions. Uses OMNIVOICE as the
    system identifier and audio IDs in the 12000000+ range to avoid
    collisions with other attack pipelines in this project.
    ...
    def __init__(
        ...
        self.system_id = system_id or settings.OMNIVOICE_SYSTEM_ID
    ...
    def execute(self) -> FormattingResult:
        ...
        if validated_path.exists():
            ...
            logger.error(
                "No samples to format (neither validated_samples.json nor "
                "generation_metadata.json found)"
            )
