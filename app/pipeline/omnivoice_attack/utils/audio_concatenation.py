...
Reference clips for OmniVoice's diffusion language model must end on a clean
silence boundary, not mid-phrase. The diffusion conditioning attempts to
"complete" abrupt cuts in the reference, manifesting as a fragment of the
reference voice at the start of generated audio (the prefix-bleed artifact
documented in the OmniVoice attack pipeline). The strategy here is:

    1. Concatenate complete files only -- never slice mid-file unless the
       first file alone exceeds the target duration.
    2. When the first file alone overflows, snap the cut to the nearest
       silent frame within +/- 1 s of the target.
    3. Always append a 200 ms trailing silence so the reference ends on a
       clear silence pattern regardless of where the last file ends.
"""
import numpy as np
import librosa
...
def concatenate_with_padding(
    ...
    Args:
        ...
        target_duration: Target duration of the speech body in seconds.
            OmniVoice docs recommend 3-10 s; longer references degrade
            cloning quality.
