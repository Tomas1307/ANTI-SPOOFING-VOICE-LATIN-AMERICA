...
from app.pipeline.openvoice_attack.steps import (
...
class OpenVoiceAttackPipeline:
    ...
    3. Generate synthetic speech (MeloTTS base + ToneColorConverter, fully local).
    ...
    def __init__(self, config: OpenVoicePipelineConfig | None = None):
        """Initialize OpenVoice attack pipeline.

        Args:
            config: Pipeline configuration (default: run all steps).
        """
        self.config = config or OpenVoicePipelineConfig()
        logger.info("OpenVoiceAttackPipeline initialized")
    ...
    def run(self) -> Path:
        ...
        """
        logger.info("=" * 80)
        logger.info("OPENVOICE ATTACK PIPELINE - START")
        logger.info("=" * 80)
        ...
        logger.info(f"Conversion tau  : {settings.CONVERSION_TAU}")
        logger.info("")
        ...
        if self.config.run_step_3:
            logger.info("STEP 3/5: Generate Synthetic Speech (MeloTTS + ToneColorConverter)")
            logger.info("-" * 80)
        ...
        logger.info("=" * 80)
        logger.info("OPENVOICE ATTACK PIPELINE - COMPLETE")
        logger.info("=" * 80)
