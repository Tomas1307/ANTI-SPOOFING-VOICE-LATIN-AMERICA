...
from app.pipeline.outetts_attack.utils.audio_concatenation import concatenate_with_padding


class ReferenceAudioPreparator:
    """Prepares reference audio clips for OuteTTS voice cloning.

    Concatenates multiple training samples per speaker to create reference
    clips used for speaker profile creation via interface.create_speaker().
    OuteTTS extracts a JSON speaker profile (tempo, energy, pitch, spectral
    centroid) from the reference waveform.
    ...
    def execute(self) -> ReferenceResult:
        ...
        """
        logger.info("Step 1: Preparing reference audio clips for OuteTTS...")
