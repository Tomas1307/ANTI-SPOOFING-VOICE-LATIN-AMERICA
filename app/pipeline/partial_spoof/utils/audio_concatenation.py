...
Reference clips passed to TTS attack strategies (especially OmniVoice's
diffusion language model) must end on a clean silence boundary, not
mid-phrase. OmniVoice in particular attempts to "complete" abrupt
mid-word cuts in the reference, manifesting as hallucinated content at
the start of generated audio (the prefix-bleed / wild-hallucination
artifact documented in `omnivoice_attack/utils/audio_concatenation.py`).

The strategy here is shared verbatim with the standalone OmniVoice
attack pipeline so partial_spoof clones do not suffer the same bug:
...
Kept local to partial_spoof (not imported from omnivoice_attack) so the
two pipelines remain decoupled at the package boundary.
"""
import numpy as np
import librosa
...
def concatenate_with_padding(
    ...
    diffusion conditioning to avoid prefix-bleed artifacts in generation
    and harmless for other strategies (Qwen, FishGram, etc.) that simply
    ignore trailing silence in the reference.
    ...
    Args:
        ...
        target_duration: Target duration of the speech body in seconds.
            Default 15.0 to preserve the prior partial_spoof behavior;
            OmniVoice's own attack pipeline uses 10.0.
