...
training samples, then transcribes them using faster-whisper for full
voice cloning mode in Qwen3-TTS.
"""
import json
...
from app.pipeline.qwen_attack.utils.audio_concatenation import concatenate_with_padding
from app.pipeline.qwen_attack.utils.reference_transcriber import transcribe_audio


class ReferenceAudioPreparator:
    """Prepares reference audio clips with STT transcription for Qwen voice cloning.
    ...
    are stored in metadata and used by Step 3 for full voice cloning mode
    (x_vector_only_mode=False), which produces higher quality clones than
    embedding-only mode.
    ...
    def execute(self) -> ReferenceResult:
        ...
        """
        logger.info("Preparing reference audio clips with STT transcription...")
