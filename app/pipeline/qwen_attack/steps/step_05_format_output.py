...
Uses QWEN3TTS system ID and 8000000+ audio ID range to avoid collisions
with FishGram pipeline outputs (9000000+ range).
"""
import json
import librosa
...
class OutputFormatter:
    ...
    following ASVspoof2019 LA naming conventions. Uses QWEN3TTS as the
    system identifier and audio IDs in the 8000000+ range to avoid
    collisions with FishGram outputs.
    ...
    def __init__(
        ...
        self.system_id = system_id or settings.QWEN_SYSTEM_ID
    ...
    def execute(self) -> FormattingResult:
        ...
        if validated_path.exists():
            ...
            logger.warning(
                f"Validation skipped - using all generated samples "
                f"from {generation_path}"
            )
